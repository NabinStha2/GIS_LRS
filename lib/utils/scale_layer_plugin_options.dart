...
class ScaleLayerWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final start = map?.project(center ?? LatLng(0, 0));
    final targetPoint = util.calculateEndingGlobalCoordinates(
        center ?? LatLng(0, 0), 90, distance);
    final end = map?.project(targetPoint);
    final displayDistance = distance > 999
        ? '${(distance / 1000).toStringAsFixed(0)} km'
        : '${distance.toStringAsFixed(0)} m';
    final width = ((end?.x ?? 0) - (start?.x as double));
